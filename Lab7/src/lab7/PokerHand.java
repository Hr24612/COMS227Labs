...
public class PokerHand
{
  ...
  public boolean isFullHouse()
  {
    ...
    if ((cards[0].getRank() == cards[1].getRank() && cards[2].getRank() == cards[3].getRank() && cards[3].getRank() == cards[4].getRank()) || 
    		(cards[0].getRank() == cards[1].getRank() && cards[2].getRank() == cards[1].getRank() && cards[3].getRank() == cards[4].getRank()))
    {
    	return true;
    }
    return false;
  ...
  private void sortNoSuit()
  {
	  for (int i = 0; i < cards.length - 1; ++i)
	    {
	      // Step 1: find the "minimal" element to the right of position i
	      int indexOfMin = findIndexOfMin2(i);
  ...
  private int findIndexOfMin2(int start)
  {
	    int indexOfMin = start;
	    for (int j = start + 1; j < cards.length; ++j)
	    {
